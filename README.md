...
- [ ] Explain what a token is used for. -- A token is used to determine whether or not someone rightfully has access to a certain page. The token authenticates them, i.e. if they have the token then they have the right credentials.
- [ ] What steps can you take in your web apps to keep your data secure? -- I don't know if we covered it, but I think you are supposed to encrypt stuff using hash functions, i.e. the hash of a password is stored and not the password itself.
- [ ] Describe how web servers work. -- They hold data on it and send data to users over a network.
